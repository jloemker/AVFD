  ...
  for (int nthOutput = 0; nthOutput < nFinalFiles; nthOutput++) {
	  ...
	  for (int ithJob = jobNum; ithJob <= jobNum+(nJob/nFinalFiles)+(nJob%nFinalFiles); ithJob++) {
		...
		if(dirID == 0){//only 30_40 exists!
		  directory = Form("/dcache/alice/jlomker/sim/tau_init0.4/BField0.4/Centrality%d_%d/job-%d/Result/event-1/particle_distribution_final/%d.dat",val1,val2,ithJob,ithFile);
