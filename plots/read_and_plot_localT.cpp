...
void read_and_plot_localT() {
	...
	const Int_t nTau = 40;//from 0.6 to 1 in dtau = 0.01
	...
	for (int tauID = 0; tauID <= nTau; tauID++){
		for (int centID = 0; centID < 8; centID++) {
			if (centID == 0 || centID == 1) {
				Tlocal2D[centID][tauID] = new TProfile2D(Form("Tlocal2D_tau%f_cent%d_%d", 0.6+(tauID*dTau),  (centID)*5, (centID+1)*5), Form("Tlocal2D__tau%f_cent%d_%d", 0.6+(tauID*dTau),  (centID)*5, (centID+1)*5), 260, -13, 13, 260, -13, 13);
			} else {
				Tlocal2D[centID][tauID] = new TProfile2D(Form("Tlocal2D_tau%f_cent%d_%d", 0.6+(tauID*dTau),  (centID-1)*10, (centID)*10), Form("Tlocal2D_tau%f_cent%d_%d", 0.6+(tauID*dTau),  (centID-1)*10, (centID)*10), 260, -13, 13, 260, -13, 13);
			}
		}
	}	
	...
	for (int centID = 0; centID < 8; centID++) {
		for (int tauID = 0; tauID <= 40; tauID++){
